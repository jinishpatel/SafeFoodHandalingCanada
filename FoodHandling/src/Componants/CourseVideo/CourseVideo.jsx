...
export const CourseVideo = () => {
	...
	const videoUrl1 =
		'https://drive.google.com/uc?id=1XbXduXzDPMpt90hZB7vWmrJaWCLaI5yP';
	const videoUrl2 =
	...
	const fetchUserPaymentStatus = async (id) => {
		try {
			const res = await axios.get(
				`http://localhost:4000/stripe/status/${id}`,
			);
			if (res.status === 200) {
				console.log('User Payment verified.');
				try {
					const res2 = await axios.get(
						`http://localhost:4000/stripe/get-certificate/${id}`,
					);
					if (res2.status === 200) {
	...
	const handleSubmit = async () => {
		...
		const percentage = ((score / 15) * 100).toFixed(2);
		...
		if (percentage >= 60) {
			...
			const res = await axios.post(
				'http://localhost:4000/stripe/create-certificate',
				data,
			);
			if (res.status === 200) {
				setShowCertificate(true);
	...
	return (
		<>
			<Navbar />
			<div className="course-certificate-quiz">
					...
					<>
						{showQuiz ? (
								...
								{quizFailed ? (
									<>
										...
										<div className="quiz-failed">
											We are really sorry that you wern't able to
											pass this quiz to get certificate. Please try
											again and we wish you very best of luck!
										</div>
									</>
								) : showCertificate ? (
									<div className="certificate">
										...
										<PDFDownloadLink
											document={<PDF userData={certificateData} />}
									...
									<>
										...
										<div className="para-1">
											<b>
												Please fill out this quiz and score at least
												60% to get Cerificate.
											</b>
										</div>
							...
							<div className="course-video">
								...
								<div className="para-1">
									<b>
										Please watch these videos to move on to next quiz
										section.
									</b>
								</div>
								<div className="progress-bar-div">
								...
								<div className="para">
									Here is your {`${getVideoNumber()}`} course video for
									food safety so kindly watch it carefully.
								</div>
								{videoNumber === 5 && (
									<div
										className="para optional-para"
										onClick={handleVideoEnd}
									>
										You can skip this video by pressing skip button to
										move into next step.{' '}
										<BiSolidSkipNextCircle className="skip-btn" />
									</div>
								)}
								{videoNumber !== 0 && (
									<video
