...
const Course = () => {
	...
	const fetchUserPaymentStatus = async () => {
		try {
			const res = await axios.get(`http://localhost:4000/stripe/status/${userID}`);
			if (res.status === 200) {
				console.log('User Payment is already done.');
	...
	useEffect(() => {
		...
		if (isUserLoggedIn) {
			setUserID(JSON.parse(Cookies.get('userData'))._id);
		}
	}, []);
	...
	const purchase_action = async () => {
		if (isauthenticated) {
			if (userPaymentCompleted) {
				navigate('/success');
				return;
			}
			const data = {
				userID,
				item: 'Food Safety Course',
				unit_price: '10000',
			};
			try {
				const res = await axios.post('http://localhost:4000/stripe/create-checkout-session', data);
				if (res.status === 200) {
				...
				<div className="main-course-part1">
					...
					<div className="part1-right">
						...
						<button className="purchase-button" onClick={purchase_action}>
							{userPaymentCompleted ? 'View Now' : 'Buy Now'}
						</button>
					</div>
				</div>
